...
export function InvestmentModel() {
  ...
  function fmt(n: number) { return Math.round(n).toLocaleString(locale); }
  function fmtK(n: number) { return Math.round(n / 1000).toLocaleString(locale) + (language === 'cs' ? ' tis.' : 'k'); }
  function fmtM(n: number) { 
    const formatted = (n / 1000000).toFixed(1);
    return (language === 'cs' ? formatted.replace(".", ",") : formatted) + (language === 'cs' ? ' mil' : 'M'); 
  }
  ...
  for (let i = 0; i <= 10; i++) {
    ...
    years.push({
      year: i,
      label: i === 0 ? (language === 'cs' ? 'Start' : language === 'it' ? 'Inizio' : 'Start') : (language === 'cs' ? 'R' : 'Y') + i,
      propValue: propValue,
      cumRent: cumRent,
      capGain: capGain,
  ...
  const returnPctFormatted = language === 'cs' ? returnPct.replace(".", ",") : returnPct;
  ...
  function cx(i: number) { return padL + (i / 10) * plotW; }
  function cy(v: number) { return padT + plotH - (v / maxChart) * plotH; }
  ...
  const baseAreaPath = "M" + cx(0) + "," + cy(price) + "L" + cx(10) + "," + cy(price) + "L" + cx(10) + "," + cy(0) + "L" + cx(0) + "," + cy(0) + "Z";
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "0 24px 24px" }}>
        ...
        <div style={{ display: "flex", gap: 16, flexWrap: "wrap", marginBottom: 24 }}>
          <div style={{ flex: "1 1 200px", background: "rgba(255,255,255,0.04)", borderRadius: 12, padding: "14px 18px" }}>
            <div style={{ fontSize: 10, color: "rgba(255,255,255,0.4)", letterSpacing: 2, marginBottom: 6 }}>{t.calculator.propertyPrice.toUpperCase()}</div>
          ...
          <div style={{ flex: "1 1 160px", background: "rgba(255,255,255,0.04)", borderRadius: 12, padding: "14px 18px" }}>
            <div style={{ fontSize: 10, color: "rgba(255,255,255,0.4)", letterSpacing: 2, marginBottom: 6 }}>{t.calculator.rentalYield.toUpperCase()}</div>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <div style={{ flex: "1 1 160px", background: "rgba(255,255,255,0.04)", borderRadius: 12, padding: "14px 18px" }}>
            <div style={{ fontSize: 10, color: "rgba(255,255,255,0.4)", letterSpacing: 2, marginBottom: 6 }}>{t.calculator.propertyAppreciation.toUpperCase()}</div>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        ...
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 28 }}>
          {[
            { label: t.calculator.propertyPrice, value: fmtM(price), color: "rgba(255,255,255,0.6)", sub: "CZK" },
            { label: t.calculator.yearlyRent, value: fmtK(annualRent), color: GOLD, sub: "CZK" },
            { label: t.calculator.totalRent + " 10 " + t.calculator.years, value: fmtM(final.cumRent), color: GOLD, sub: "CZK" },
            { label: t.calculator.finalValue + " 10 " + t.calculator.years, value: fmtM(final.propValue), color: "#8B9FFF", sub: "CZK", bold: true },
            { label: t.investorBenefits.capitalGrowth, value: "+" + fmtM(final.capGain), color: "#8B9FFF", sub: "CZK" },
            { label: t.calculator.totalProfit.toUpperCase(), value: fmtM(final.totalProfit), color: O, sub: "CZK", big: true },
            { label: t.calculator.roi.toUpperCase(), value: returnPctFormatted + " %", color: O, sub: "", big: true },
          ].map((k) => {
            return (
        ...
        <div style={{ marginBottom: 28 }}>
          <div style={{ fontSize: 10, color: "rgba(255,255,255,0.4)", letterSpacing: 2, marginBottom: 8 }}>
            {language === 'cs' ? 'ROZDĚLENÍ ZISKU' : language === 'it' ? 'RIPARTIZIONE PROFITTO' : 'PROFIT BREAKDOWN'}
          </div>
          <div style={{ display: "flex", borderRadius: 8, overflow: "hidden", height: 36 }}>
            <div style={{
              ...
              {(language === 'cs' ? 'Nájem: ' : language === 'it' ? 'Affitto: ' : 'Rent: ') + (final.cumRent / final.totalProfit * 100).toFixed(1).replace(".", language === 'cs' ? ',' : '.') + " %"}
            </div>
            <div style={{
              ...
              {(language === 'cs' ? 'Růst ceny: ' : language === 'it' ? 'Crescita: ' : 'Growth: ') + (final.capGain / final.totalProfit * 100).toFixed(1).replace(".", language === 'cs' ? ',' : '.') + " %"}
            </div>
          </div>
        ...
        <div style={{ background: "rgba(255,255,255,0.03)", borderRadius: 16, padding: "20px 16px 12px", marginBottom: 24 }}>
          <svg width="100%" viewBox={"0 0 " + chartW + " " + chartH} style={{ display: "block" }}>
            ...
            <text x={chartW - 203} y={20} fill="rgba(255,255,255,0.6)" fontSize="11" fontFamily="system-ui">
              {language === 'cs' ? 'Celková hodnota (nemov. + nájem)' : language === 'it' ? 'Valore totale (immobile + affitto)' : 'Total value (property + rent)'}
            </text>
            <rect x={chartW - 220} y={28} width="12" height="12" rx="2" fill="#5B6ABF" />
            <text x={chartW - 203} y={38} fill="rgba(255,255,255,0.6)" fontSize="11" fontFamily="system-ui">
              {language === 'cs' ? 'Hodnota nemovitosti' : language === 'it' ? 'Valore immobile' : 'Property value'}
            </text>
            <rect x={chartW - 220} y={46} width="12" height="3" fill="rgba(255,255,255,0.15)" />
            <text x={chartW - 203} y={52} fill="rgba(255,255,255,0.4)" fontSize="10" fontFamily="system-ui">
              {language === 'cs' ? 'Kupní cena' : language === 'it' ? 'Prezzo d\'acquisto' : 'Purchase price'}
            </text>
          </svg>
        </div>
        ...
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead>
              <tr>
                {[
                  language === 'cs' ? 'ROK' : language === 'it' ? 'ANNO' : 'YEAR',
                  language === 'cs' ? 'HODNOTA NEMOV.' : language === 'it' ? 'VALORE IMMOBILE' : 'PROPERTY VALUE',
                  language === 'cs' ? 'ROČNÍ NÁJEM' : language === 'it' ? 'AFFITTO ANNUO' : 'ANNUAL RENT',
                  language === 'cs' ? 'KUM. NÁJEM' : language === 'it' ? 'AFFITTO CUM.' : 'CUM. RENT',
                  language === 'cs' ? 'KAPITÁL. ZISK' : language === 'it' ? 'GUADAGNO CAP.' : 'CAPITAL GAIN',
                  language === 'cs' ? 'CELKOVÝ ZISK' : language === 'it' ? 'PROFITTO TOTALE' : 'TOTAL PROFIT'
                ].map((h, i) => {
                  const colors = ["rgba(255,255,255,0.4)", "#8B9FFF", GOLD, GOLD, "#8B9FFF", O];
                  return (
                    <th key={h} style={{
        ...
        <div style={{ marginTop: 20, padding: "14px 0", borderTop: "1px solid rgba(255,255,255,0.06)", display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
          <span style={{ fontSize: 11, color: "rgba(255,255,255,0.25)" }}>{"SPILBERK SICAV | RENTBACK by SPILBERK"}</span>
          <span style={{ fontSize: 11, color: "rgba(255,255,255,0.25)" }}>
            {language === 'cs' ? 'Modelový příklad, skutečné výnosy se mohou lišit' : language === 'it' ? 'Esempio di modello, i rendimenti effettivi possono variare' : 'Model example, actual returns may vary'}
          </span>
        </div>
      </div>
    </div>
